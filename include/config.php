...
class Config {
	...
	public function __construct () {
		...
		$this->eLink = MAIN_URL.'/realestate';
...
//			$row['username'] = $row['oauth_uid'];
	...
	function addCoinHistory ($from, $to, $coins, $reason) {
		$query = "INSERT INTO coins_history SET from_uid = ?, to_uid = ?, coins = ?, reason = ?";
		$stmt = $this->conn->prepare($query);

		$stmt->bindParam(1, $from);
		$stmt->bindParam(2, $to);
		$stmt->bindParam(3, $coins);
	...
	function addCoin ($coin, $u = null, $from = 0, $reason = null) {
		if ($coin) {
			if ($u) {
				// get coin
			...
			if ($stmt->execute()) {
				if ($u && $coin && $reason) $this->addCoinHistory($from, $u, $coin, $reason);
				return true;
			}
			else return false;
		}
